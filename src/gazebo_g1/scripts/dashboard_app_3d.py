...
csv_path = os.path.join(script_dir, "..", "results", "localization_results.csv")

try:
    robot_df = pd.read_csv(csv_path, sep=",")
    robot_df.columns = [col.strip() for col in robot_df.columns]
    
    # Assicurati che i nomi delle colonne corrispondano al nuovo script
    for col in ["Time_s", "Error_Translation_m", "Error_Rotation_deg", "GT_X", "GT_Y", "EST_X", "EST_Y"]:
    ...
    fig_map.add_trace(go.Scatter(
        x=robot_df["EST_X"], y=robot_df["EST_Y"],
        mode='lines',
        name="Estimated Path (AMCL / RTAB-Map)",
    ...
    for i in range(0, len(robot_df), step):
        ...
        if row["Error_Translation_m"] > 0.5:
            error_lines_x.extend([row["GT_X"], row["EST_X"], None])
            error_lines_y.extend([row["GT_Y"], row["EST_Y"], None])
    ...
    fig_error.add_trace(go.Scatter(x=robot_df["Time_s"], y=robot_df["Error_Translation_m"], name="Translational Error (m)", line=dict(color="red", width=2)), secondary_y=False)
    fig_error.add_trace(go.Scatter(x=robot_df["Time_s"], y=robot_df["Error_Rotation_deg"], name="Rotational Error (°)", line=dict(color="purple", width=2, dash="dot")), secondary_y=True)

    fig_error.update_layout(
        title_text="Error Trend Over Time", xaxis_title="Time (s)", template="plotly_white", height=400, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
    )
    ...
    print("ERROR: CSV file not found. You must first run a test to generate the data!")
